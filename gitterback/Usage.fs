...
let usage focus =
  ...
  if focus = "" || focus = "setup" then
    cp ""
    cp "\fogitterback \fysetup \fg-a \fc<anchor> \fg-n \fc<name>\f0"
    cp "   Create a new bare repository in the given \fc<anchor>\f0 folder,"
    cp "   with the given \fc<name>\f0 to backup the current GIT repository."
    cp "   The \fc<name>\f0 is the name of the bare repository, without the .git suffix."
    cp "   The \fc<anchor>\f0 folder must have been previously registered with "
    cp "   the \fyanchor create\f0 command."
    cp "   This command will also register anew remote named \fc<anchor>\f0 in"
